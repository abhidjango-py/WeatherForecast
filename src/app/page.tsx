...
// https://api.openweathermap.org/data/2.5/forecast?q=kanpur&appid=38315fa1753cc4cee0b7e63db3484b56&cnt=56
      ...
      <main className="px-3 max-w-7xl mx-auto flex flex-col gap-9 w-full pb-10 pt-4">
        ...
        <Container className="gap-10 px-6 items-center">
          ...
          <div className="flex gap-4">
              ...
              <Container className="bg-yellow-300/80 px-14 gap-4 justify-between overflow-x-auto" >
              <WeatherDetails visibility={mtrToKm(firstData?.visibility ?? 10000)} airPressure={`${firstData?.main.pressure} hPa`} humidity={`${firstData?.main.humidity}%`}
                sunrise={format(fromUnixTime(data?.city.sunrise ?? 1702949452),"H:mm")} sunset={format(fromUnixTime(data?.city.sunset ?? 1702949452),"H:mm")} windSpeed={convertWindSpeed(firstData?.wind.speed ?? 1.64)}    />
              </Container>
          </div>
          </section>
        {/*7 days forecast data*/}
        <section className="flex w-full flex-col gap-4">
          <p className="text-2xl">Further Forecast</p>
          {firstDataForEachDate.map((d,i)=>(
               ...
               visibility={`${mtrToKm(d?.visibility ?? 10000)}`}
